...
void find_empty_page(Table *table) {
    for (int i = 0; i < max_pages; i++) {
        ...
        if (table->pages[i]->num_rows < rows_per_page) {
            //printf("Found non-full page at index %d.\n", i);
            find_empty_row(table->pages[i]);
            return;
        }
    }
